...
- 追高约束曾于 2026-09-08 由 `18%` 收紧为 `12%`，同日回退。收紧时的采纳依据是开发池同公式盈亏比 `+4.29%`，但在现已揭示的 230 只池外股票上该增益仅 `+0.11%`，且池外平均收益 `-4.92pp`、A 口径组合 CAGR `-0.41pp`；NEW122 收益中位数由 `52.03%` 降至 `46.46%`。依据不成立，已恢复 `18%`。收紧版报告保留为 `offline_backtest_report_20260908_205428.txt`（ALL305）与 `_205312.txt`（DEV250）。
- 当前 ALL305 五项主指标为平均逐日盯市回撤 `-48.89%`、盈利股票占比 `73.77%`、收益中位数 `43.77%`、平均胜率 `37.67%`、同公式盈亏比 `1.94`；平均收益 `138.76%` 只作右尾辅助指标。
- 当前 `18%` 策略在已揭示 OOS230 上的研究 harness 结果为：平均收益 `43.91%`、收益中位数 `7.47%`、盈利股票占比 `56.52%`、同公式盈亏比 `1.393`、A 口径组合 CAGR `6.07%`、Sharpe `0.705`。这些数字明显弱于开发池，但数据源和行业风格也不同，只能作为泛化风险信号，不能单独证明时间外失效。
- 已揭示 OOS230 位于 `stock_trading_advisor/data/oos_data/`（`150` A 股 + `80` 港股，与开发池零重叠），选池规则、数据来源和限制见 `stock_trading_advisor/oos_benchmark/manifest.json`。它已经参与过候选取舍，只能用于回归检查，禁止在其上调参或再次作为未见验收池。
